...
class RadioDeApi():
    ...
    USER_AGENT = 'XBMC Addon Radio'    #since 05.02.2015, Mopidy User Agent does not work any longer...

    def __init__(self, language='german', user_agent=USER_AGENT, proxy=None):
    ...
    def parse_playlist(self, stream_url):
        ...
        if stream_url.lower().endswith('m3u'):
            ...
            print("Found pls file", stream_url)
            response = self.__urlopen(stream_url)
            #response = urlopen(stream_url)
    ...
    def __api_call(self, path, param=None):
        #print('__api_call started with path=%s, param=%s'% (path, param))
    ...
    def __urlopen(self, url):
        print('__urlopen opening url=%s'% url)
        req = Request(url)
        ...
        while (downloadloop is True):
            ...
            if response != {}:
                ...
                i += 1
                print("Try to download again. Loop No:", i)
                time.sleep(0.5)
                if i > 3:
                    print("Unable to download from www.radio.de .... damn")
                    return False