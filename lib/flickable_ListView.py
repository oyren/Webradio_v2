...
class Flickable_ListWidget(QListWidget):
    ...
    def handleScroll(self, startPoint, deltaPoint):
        if self.startPoint_stored != startPoint:
            self.startPoint_stored = startPoint
            self.oldValue = self.verticalScrollBar().value()
        else:
            print("Startpoint has not moved")
            #self.oldValue = self.verticalScrollBar().value()
            newValue = self.oldValue - deltaPoint.y()

            print("received:", startPoint.y(), deltaPoint.y())
            if deltaPoint.y() > startPoint.y():
                print("down")
            else:
                print("up")
    ...
    def eventFilter(self, Object, Event):
        ...
        if Object == self.viewport():

            if Event.type() == QEvent.MouseButtonPress:
                print("Press at",QPoint(Event.pos()))
                self.startPoint = QPoint(Event.pos())
                self.locked = True

            if Event.type() == QEvent.MouseButtonRelease:
                print("Release at",QPoint(Event.pos()))
                self.EndPoint = QPoint(Event.pos())
                self.locked = False

            if Event.type() == QEvent.MouseMove:
                if self.locked:
                    print("Move", self.startPoint, QPoint(Event.pos()))
...
if __name__ == "__main__":
    ...
    colorList = Flickable_ListWidget()
    colorList.setWindowTitle("Kinetic Scrolling")

    #for i in range(50):
    #    newItem = QListWidgetItem(colorList)  # create new ListWidgetItem (parent= ListWidget)
    #    newItem.setText("This is entry no {0}".format(i))
#ifdef Q_OS_SYMBIAN
